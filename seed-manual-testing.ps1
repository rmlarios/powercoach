...
function Post-Api($url, $body = $null) {
    if ($null -eq $body) {
        ...
        Invoke-RestMethod -Uri $url -Method POST -Headers $headers -Body ($body | ConvertTo-Json -Depth 10 -Compress) -ContentType "application/json"
    }
}

function Put-Api($url, $body) {
    Invoke-WebRequest -Uri $url -Method PUT -Headers $headers -Body ($body | ConvertTo-Json -Depth 10 -Compress) -ContentType "application/json" -UseBasicParsing | Out-Null
}
...
for ($i = 0; $i -lt 3; $i++) {
    ...
    $athId = if ($result.athleteId) { [string]$result.athleteId } else { Get-Id $result }
    $athleteIds += $athId
